...
var correctionInterval = 3;
    ...
    function correctPoints(){
        for (let i = 0; i < counter; i++) {
            ...
            trkHist = document.getElementById("track" + i.toString());

            //console.log(trkHist.getAttribute('lat'));
            trkLat = parseFloat(trkHist.getAttribute('lat'));
            trkLon = parseFloat(trkHist.getAttribute('lon'));
            trkPoint = SkyVector.ll2xy(trkLat,trkLon);
            trkHist.style.top = Math.round(trkPoint.y - SkyVector.data.slideroffsety - trackSize/2) + "px";
            trkHist.style.left = Math.round(trkPoint.x - SkyVector.data.slideroffsetx - trackSize/2) + "px";
          }
    ...
    function setMapIcon(latlon) 
    {
        ...
        lastLatLon = latlon;
        lastPoint = SkyVector.ll2xy(lastLatLon[0], lastLatLon[1]);
        curPoint = point;
        lastHeading = heading;
        //SkyVector.drawCanvasBarb(lastLatLon[0],lastLatLon[1],lastHeading,100);
        if(SkyVector.data.tilesize != tileSize){
    ...
    function setMapTrack(latlon) 
    {
        if (typeof SkyVector === 'undefined')
        {
            ...
            trk = document.createElement("img");
            trk.id = "track" + counter;
            counter= counter + 1;
            trk.src=planeIco;
            trk.style.height = trackSize + "px";
            trk.style.width = trackSize + "px";
            trk.style.zIndex = "10";
            trk.style.position = "absolute";
            var el = document.getElementById("sv_svgLayer");
            el.parentNode.insertBefore(trk, el.nextSibling);
      

        var point = lastPoint;
        trk.style.top = Math.round(point.y - SkyVector.data.slideroffsety - trackSize/2) + "px";
        trk.style.left = Math.round(point.x - SkyVector.data.slideroffsetx - trackSize/2) + "px";
        var heading = lastHeading;
         trk.style.transform = "rotate(" + heading + "deg)";
        trk.style.visibility = "visible";
        trk.setAttribute('lat', lastLatLon[0]);
        trk.setAttribute('lon', lastLatLon[1]);
        //trk.setAttribute('tileSize',parseInt(SkyVector.data.tilesize));
    
       

        //  var tc = counter -1; 
       // lastTrk = document.getElementById("track" + tc.toString());

       // lastTrkSize = parseInt(lastTrk.getAttribute('tileSize'));
        //console.log(lastTrkSize);
        //console.log(parseInt(SkyVector.data.tilesize));
